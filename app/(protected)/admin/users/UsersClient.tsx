...
const AVATAR_HEX = ["#3b82f6","#8b5cf6","#10b981","#f59e0b","#ec4899","#14b8a6","#6366f1"];

function Avatar({ name, username }: { name: string; username: string }) {
  const initials = name
    ? name.split(" ").map((w) => w[0]).join("").toUpperCase().slice(0, 2)
    : username.slice(0, 2).toUpperCase();
  const bg = AVATAR_HEX[username.charCodeAt(0) % AVATAR_HEX.length];
  return (
    <div style={{ background: bg }} className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-sm font-bold text-white select-none">
      {initials}
    </div>
  );
}
...
function StatusBadge({ enabled }: { enabled: boolean }) {
  return enabled ? (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700 ring-1 ring-inset ring-green-600/20">
      <span className="h-1.5 w-1.5 rounded-full bg-green-500" />啟用
    </span>
  ) : (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-700 ring-1 ring-inset ring-red-600/20">
      <span className="h-1.5 w-1.5 rounded-full bg-red-500" />停用
    </span>
  );
}
...
function Toast({ message, type, onClose }: { message: string; type: ToastType; onClose: () => void }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-2xl px-5 py-3.5 text-sm font-medium text-white shadow-2xl ${type === "success" ? "bg-emerald-600" : "bg-red-600"}`}>
      {type === "success"
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between border-b border-gray-100 px-6 py-5">
          ...
          <button onClick={onClose} className="ml-4 flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
...
function UserForm({ form, isEdit, saving, error, onChange, onSubmit, onCancel }: {
  form: FormState; isEdit: boolean; saving: boolean; error: string;
  onChange: (f: Partial<FormState>) => void; onSubmit: () => void; onCancel: () => void;
}) {
  ...
  return (
    <div className="space-y-4">
      {!isEdit && (
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-700">帳號 <span className="text-red-500">*</span></label>
          <Input value={form.username} onChange={(v) => onChange({ username: v })} placeholder="例：john.doe" />
        </div>
      )}
      <div>
        <label className="mb-1.5 block text-sm font-medium text-gray-700">Email</label>
        <Input value={form.email} onChange={(v) => onChange({ email: v })} type="email" placeholder="user@hospital.local" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-700">姓（Last）</label>
          <Input value={form.lastName} onChange={(v) => onChange({ lastName: v })} placeholder="王" />
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-700">名（First）</label>
          <Input value={form.firstName} onChange={(v) => onChange({ firstName: v })} placeholder="小明" />
        </div>
      </div>
      <div>
        ...
        <div className="relative">
          <Input value={form.password} onChange={(v) => onChange({ password: v })} type={showPwd ? "text" : "password"} placeholder={isEdit ? "（留空不變）" : "請輸入密碼"} />
        ...
        {isEdit && <p className="mt-1 text-xs text-gray-400">留空則不變更密碼</p>}
      </div>
      <div className="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
        <div>
          <p className="text-sm font-medium text-gray-700">帳號狀態</p>
          <p className="text-xs text-gray-400">{form.enabled ? "目前為啟用狀態" : "已停用，使用者無法登入"}</p>
        </div>
        <button type="button" onClick={() => onChange({ enabled: !form.enabled })}
          className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${form.enabled ? "bg-blue-600" : "bg-gray-300"}`}>
      ...
      <div className="flex items-center justify-end gap-2 border-t border-gray-100 pt-4">
        <button onClick={onCancel} className="rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">取消</button>
        <button onClick={onSubmit} disabled={saving} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50 transition-colors">
          {saving && <Spinner />}{saving ? "儲存中…" : "儲存"}
        </button>
      </div>
    </div>
  );
...
export default function UsersClient({ initialUsers }: { initialUsers: KeycloakUser[] }) {
  ...
  const refreshList = async (q?: string) => {
    setLoading(true);
    try {
      const url = q ? `/api/bff/users?search=${encodeURIComponent(q)}` : "/api/bff/users";
      const res = await fetch(url);
      if (res.ok) setUsers(await res.json());
    } finally { setLoading(false); }
  };

  const handleSearch = useCallback(() => refreshList(search), [search]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl bg-blue-50 px-5 py-4">
          <p className="text-xs font-medium text-blue-600 uppercase tracking-wide">總使用者</p>
          <p className="mt-1 text-3xl font-bold text-blue-700">{users.length}</p>
        </div>
        <div className="rounded-2xl bg-emerald-50 px-5 py-4">
          <p className="text-xs font-medium text-emerald-600 uppercase tracking-wide">啟用中</p>
          <p className="mt-1 text-3xl font-bold text-emerald-700">{enabledCount}</p>
        </div>
        <div className="rounded-2xl bg-red-50 px-5 py-4">
          <p className="text-xs font-medium text-red-500 uppercase tracking-wide">已停用</p>
          <p className="mt-1 text-3xl font-bold text-red-600">{users.length - enabledCount}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          <svg className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          <input ref={searchRef} type="text" placeholder="搜尋帳號或 Email…" value={search}
            onChange={(e) => setSearch(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleSearch()}
            className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-10 pr-10 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
          />
          {search && (
            <button onClick={() => { setSearch(""); refreshList(); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          )}
        </div>
        <button onClick={handleSearch} disabled={loading} className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-50 transition-colors">
          {loading ? <Spinner /> : <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>}
          搜尋
        </button>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        {users.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="font-medium text-gray-700">找不到使用者</p>
            <p className="mt-1 text-sm text-gray-400">{search ? "嘗試不同的搜尋關鍵字" : "點擊「新增使用者」建立第一個帳號"}</p>
          </div>
        ) : (
          <>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">使用者</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Email</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">狀態</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {users.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="group transition-colors hover:bg-blue-50/50">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <Avatar name={fullName} username={u.username} />
                          <div className="min-w-0">
                            <p className="font-semibold text-gray-900">{u.username}</p>
                            {fullName && <p className="truncate text-xs text-gray-400">{fullName}</p>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(u)} title="編輯"
                          ...
                          <button onClick={() => setDeleteTarget(u)} title="刪除"
            ...
            <div className="border-t border-gray-100 bg-gray-50/50 px-5 py-3">
              <p className="text-xs text-gray-400">共 <span className="font-semibold text-gray-600">{users.length}</span> 位使用者</p>
            </div>
          </>
        )}
      </div>
        ...
        <Modal title="確認刪除" onClose={() => setDeleteTarget(null)}>
          <div className="space-y-5">
            <div className="flex items-start gap-4 rounded-xl bg-red-50 p-4 border border-red-100">
              ...
              <div>
                <p className="font-semibold text-red-800">此操作無法復原</p>
                <p className="mt-1 text-sm text-red-700">確定要永久刪除使用者 <span className="font-bold">@{deleteTarget.username}</span>？</p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleDelete} disabled={saving} className="flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-50 transition-colors">
                {saving && <Spinner />}{saving ? "刪除中…" : "確認刪除"}
              </button>
            </div>
          </div>
        </Modal>
